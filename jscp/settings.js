...
let settings = {
  music: "./music/happy-birthdaay.mp3",
  countdown: 3,
...
const musicOptions = [
  { value: "./music/happy-birthdaay.mp3", label: "Happy Birthday (Miễn phí)" },
  {
  ...
  { value: "./music/happybirthdaay.mp3", label: "Happy Birthday" },
  {
    value: "./music/postcardnam.mp3",
    label: "Happy Birthday Podcast giọng nam",
  },
...
const gifOptions = [
  ...
  { value: "./gif/catlove.gif", label: "Gif2" },
  ...
  if (isSaveCheckbox && expireDateElement && expireContainer) {
    if (isSaveCheckbox.checked) {
      ...
      expireContainer.innerHTML = `⏳ ${t(
        "expireText"
      )} <b id="expireDate" style="color: #4caf50;">${t("saveForever")}</b>`;
    } else {
      expireDateElement.textContent = t("thirtyDays");
      expireDateElement.style.color = ""; // Màu mặc định
      expireContainer.innerHTML = `⏳ ${t("expireText")} <b id="expireDate">${t(
        "thirtyDays"
      )}</b>`;
    }
  }
...
async function loadWebsiteFromServer() {
  ...
  try {
    ...
    if (result.success && result.data) {
      ...
      window.settings = {
        music: serverSettings.music || "./music/happy-birthdafy.mp3",
