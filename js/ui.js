...
function checkOrientation() {
    ...
    if (!isMobile) {
        safeSet(orientationLock, 'none');
        safeSet(matrixCanvas, 'block');
        safeSet(mainCanvas, 'block');
        ...
        window.isLandscape = mediaQuery.matches;
        ...
        if (window.isLandscape) {
            ...
            if (book) book.style.display = 'block';
            if (typeof startWebsite === 'function') startWebsite();
            setTimeout(()=>{ if (typeof forceResizeMatrix === 'function') forceResizeMatrix(); }, 250);
        } else {
            safeSet(orientationLock, 'flex');
            safeSet(matrixCanvas, 'none');
            safeSet(mainCanvas, 'none');
            if (bookContainer) bookContainer.style.display = 'none';
            if (book) book.style.display = 'none';
            if (typeof stopWebsite === 'function') stopWebsite();
        }

        try {
            mediaQuery.addEventListener('change', (e)=>{
                window.isLandscape = e.matches;
                if (window.isLandscape) {
                    safeSet(orientationLock, 'none');
                    safeSet(matrixCanvas, 'block');
                    safeSet(mainCanvas, 'block');
