...
function startWebsite() {
  ...
  if (window.settings) {
    ...
    // Ждём пока settings подгрузится
...
function initMatrixRain() {
  ...
  for (let x = 0; x < columns; x++) {
    ...
    columnColors[x] =
      x % 2 === 0
        ? window.settings
          ? window.settings.matrixColor1
          : settings.matrixColor1
        : window.settings
        ? window.settings.matrixColor2
        : settings.matrixColor2;
  ...
  function drawMatrixRain() {
    ...
    matrixCtx.font =
      "bold " +
      fontSize +
      "px Menlo, Consolas, 'Liberation Mono', 'Courier New', monospace";
...
S = {
  ...
  init: function () {
    ...
    if (i !== -1) {
      ...
      console.log(
        "Simulating action from URL:",
        decodeURI(action).substring(i + 3)
      );
      ...
      const currentSettings = window.settings || {
        countdown: 3,
        sequence: "HAPPY|BIRTHDAY|PRINCESS|DINARA",
      };
      const countdownValue = currentSettings.countdown || 3;
      const sequenceText =
        currentSettings.sequence || "HAPPY|BIRTHDAY|PRINCESS|DINARA";
      const sequence = `|#countdown ${countdownValue}|${sequenceText}|#gift|`;
      S.UI.simulate(sequence);
    }

    S.Drawing.init(".canvas");
    // >>> вставить сразу после S.Drawing.init('.canvas');
    const currentSettings = window.settings || {
      countdown: 3,
      sequence: "HAPPY|BIRTHDAY|PRINCESS|DINARA",
    };
    ...
    if (S.UI && typeof S.UI.reset === "function") S.UI.reset(true);
    if (S.UI && typeof S.UI.simulate === "function") S.UI.simulate(sequence);
          ...
          case "gift":
            ...
            if (currentSettings.enableBook === true) {
              if (canvas && giftImage && matrixCanvas) {
                ...
                if (
                  giftImage.src &&
                  giftImage.src !== window.location.href &&
                  giftImage.src !== "" &&
                  !giftImage.src.includes("undefined")
                ) {
      ...
      while (n.dots.length > 0) {
        ...
        if (dots[d].s) {
          dots[d].move(
            new S.Point({
              z:
                Math.random() * (params.maxSize - params.minSize) +
                params.minSize,
              a: Math.random(),
              h: 18,
            })
...
function createHeartPhotoCentered(idx, total) {
  ...
  const targetY =
    -scale *
    (13 * cos_t - 5 * Math.cos(2 * t) - 2 * Math.cos(3 * t) - Math.cos(4 * t));
...
function checkBookFinished() {
  ...
  const lastPage = document.querySelector(
    `.page[data-page="${lastPageIndex}"]`
  );
  if (
    currentPage === lastPageIndex &&
    lastPage &&
    lastPage.classList.contains("flipped")
  ) {
    if (!isBookFinished) {
      isBookFinished = true;
      const contentDisplay = document.getElementById("contentDisplay");
...
function nextPage() {
  ...
  if (currentPage < totalPhysicalPages - 1 && !isFlipping) {
    isFlipping = true;
    const pageToFlip = document.querySelector(
...
async function showPageContent() {
  ...
  if (currentPage === 0) {
    ...
    const currentPhysicalPage = document.querySelector(
      `.page[data-page="${currentPage}"]`
    );
    if (
      currentPhysicalPage &&
      currentPhysicalPage.classList.contains("flipped")
    ) {
  ...
  if (currentPageElement && !currentPageElement.classList.contains("flipped")) {
    if (deltaX < -swipeThreshold) {
      ...
      currentPageElement.style.boxShadow = `${
        rotation / 10
      }px 10px 20px rgba(0,0,0,${0.3 + Math.abs(rotation / 100)})`;
    }
    ...
    if (
      prevPageElement &&
      prevPageElement.classList.contains("flipped") &&
      deltaX > swipeThreshold
    ) {
      prevPageElement.style.transform = `rotateY(${
        -180 + Math.abs(rotation)
      }deg)`;
      prevPageElement.style.boxShadow = `${
        -rotation / 10
      }px 10px 20px rgba(0,0,0,${0.3 + Math.abs(rotation / 100)})`;
    }
  }
